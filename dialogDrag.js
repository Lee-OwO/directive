const Z_INDEX_KEY = "V_DRAG_MAX_INDEX";
...
function addEvent(el, dragList, dragable = true) {
  dragList.forEach((item) => {
    if (dragable) {
      item.onmousedown = (e) => {
        const zIndex = window[Z_INDEX_KE] ? window[Z_INDEX_KE] : 0;
        const nowZIndex = +el.style.zIndex;
        if (zIndex !== nowZIndex) {
          const newZIndex = zIndex + 1;
          el.style.zIndex = newZIndex;
          window[Z_INDEX_KE] = newZIndex;
        }
...
export default {
  ...
  bind(el, { value }) {
    ...
    const zIndex = window[Z_INDEX_KEY];
    const newZIndex = zIndex + 1;
    el.style.zIndex = newZIndex;
    window[Z_INDEX_KEY] = newZIndex;
